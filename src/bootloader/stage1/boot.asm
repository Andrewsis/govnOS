...
start:
    ...
    ; setup stack
    mov ss, ax
    mov sp, 0x7C00 ; stack growth downwards from where we are loaded im memory

    ; Some BIOSes might start at 07C0:0000 instead of 0000:07C0.
    ; Make sure we are int the expected location
    push es
    push word .after
    retf
...
.after:
    ...
    ; compute seize of root directory = (32 * number_of_entries) / bytes_per_sector
...
.search_kernel:
    ...
    mov cx, 11                      ; compare up tp 11 characters
    push di
    repe cmpsb
    pop di
    je .found_kernel
...
.read_finish:
    ...
    jmp wait_key_and_reboot     ; should never happend
...
;
;   Converts    an LBA (Logical Block Adressing) address to
...
.loop:
    ...
    or al, al ; verify in next character is null?
    jz .done

    mov ah, 0x0e ; call bios interrupt 0x10 to print text
    mov bh, 0
    int 0x10
...
lba_to_chs:
    ...
    xor dx, dx                          ; dx = 0
    div word [bdb_sectors_per_track]    ; ab = LBA / Sectors per track
...
;   Parameters:
;       - ax: LBA address
;       - cl: number of sectors to read up (up to 128)
;       - dl: drive number
...
.retry:
    ...
    int 13h             ; carry flag set = success
    jnc .done
